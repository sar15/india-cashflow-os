"""Parser for the Cashflow OS manual template (Excel and CSV).

Accepts XLSX workbooks with Setup, Cash Events, and Obligations sheets,
or CSV files that are converted to the same format.  Each row is parsed
individually so that a single bad cell produces a row-specific error
...
from cashflow_os.utils.dates import parse_date_value, today_ist
...
def parse_manual_workbook(org_id: str, filename: str, file_bytes: bytes) -> ParsedImportBundle:
    try:
        ...
        raise FileParseError(
            "Cannot open '{filename}' as an Excel workbook. Ensure it is a valid .xlsx file.".format(filename=filename),
            filename=filename,
        ) from exc
    ...
    if "Cash Events" not in workbook.sheetnames:
        ...
        for row_index, row in enumerate(event_sheet.iter_rows(min_row=2, values_only=True), start=2):
            ...
            try:
                ...
                raw_amount = row_data.get("gross_amount_inr") or row_data.get("amount_inr") or 0
                try:
                    amount = to_minor_units(parse_indian_number(raw_amount))
                except (ValueError, TypeError) as amt_err:
                    raise FileParseError(
                        "Row {row}: Expected a number in the 'amount' column, got '{value}'.".format(
                            row=row_index, value=raw_amount
                        ),
                        filename=filename,
                        row_number=row_index,
                ...
                raw_tax = row_data.get("tax_amount_inr") or 0
                try:
                    tax_amount = to_minor_units(parse_indian_number(raw_tax))
                except (ValueError, TypeError):
                    tax_amount = 0
                    issues.append(ImportIssue(
                        code="invalid_number", severity=Severity.WARNING,
                        message="Row {row}: Could not parse tax amount '{val}', defaulting to 0.".format(row=row_index, val=raw_tax),
                        row_number=row_index, field_name="tax_amount_inr",
                    ))

                raw_tds = row_data.get("tds_amount_inr") or 0
                try:
                    tds_amount = to_minor_units(parse_indian_number(raw_tds))
                except (ValueError, TypeError):
                    tds_amount = 0
                    issues.append(ImportIssue(
                        code="invalid_number", severity=Severity.WARNING,
                        message="Row {row}: Could not parse TDS amount '{val}', defaulting to 0.".format(row=row_index, val=raw_tds),
                        row_number=row_index, field_name="tds_amount_inr",
                    ))
                ...
                raise FileParseError(
                    "Row {row}: Unexpected error — {error}".format(row=row_index, error=str(row_exc)),
                    filename=filename,
                    row_number=row_index,
                ) from row_exc
    ...
    if "Obligations" in workbook.sheetnames:
        ...
        for ob_index, row in enumerate(obligation_sheet.iter_rows(min_row=2, values_only=True), start=2):
            if not any(row):
                continue
            try:
                ...
                raise FileParseError(
                    "Obligations sheet row {row}: {error}".format(row=ob_index, error=str(ob_exc)),
                    filename=filename,
                    row_number=ob_index,
                ) from ob_exc
...
def parse_manual_csv(org_id: str, filename: str, file_text: str) -> ParsedImportBundle:
    try:
        ...
        raise FileParseError(
            "Cannot parse '{filename}' as CSV: {error}".format(filename=filename, error=str(csv_err)),
            filename=filename,
        ) from csv_err
