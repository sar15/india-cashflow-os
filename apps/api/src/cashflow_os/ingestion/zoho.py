...
def parse_zoho_payload(org_id: str, filename: str, payload: Dict) -> ParsedImportBundle:
    batch = ImportBatch(org_id=org_id, source_type=SourceType.ZOHO, filename=filename)
    ...
    for index, invoice in enumerate(payload.get("invoices", []), start=1):
        ...
        events.append(
            CanonicalCashEvent(
                ...
                notes="Imported from Zoho Books invoice payload",
            )
        )
    ...
    for index, bill in enumerate(payload.get("bills", []), start=1):
        ...
        events.append(
            CanonicalCashEvent(
                ...
                notes="Imported from Zoho Books bill payload",
            )
        )
